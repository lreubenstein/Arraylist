...
public class ScantArray {
    ...
    public void removeColumn(int col){
        ...
        while(x < entries.size()) {
            ScantArrayEntry ent = entries.get(x);
            if(ent.getColumn() == col){
                entries.remove(x);
            }
            else if (ent.getColumn() > col){
                ScantArrayEntry sa = new ScantArrayEntry(ent.getRow(),
                        ent.getColumn()-1, ent.getValue());
                entries.set(x, sa);
                x++;
            }
            else x++;
